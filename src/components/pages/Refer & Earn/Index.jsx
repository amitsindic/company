...
const ReferAndEarn = () => {
  ...
  useEffect(() => {
    if (statsUpdated) {
      ...
      const referralStats = storedReferralStats ? JSON.parse(storedReferralStats) : null;
  
      if (referralStats && referralStats.convertsList) {
        // Filter for registration events where status is not "2" and count them
        const registered = referralStats.convertsList.filter(item =>
          item.event_name === "register" && item.status !== "2"
        ).length;
        setRegisteredCount(registered);
  
        // Filter for sale events where status is not "2" and count them
        const sales = referralStats.convertsList.filter(item =>
          item.event_name === "sale" && item.status !== "2"
        ).length;
        setSaleCount(sales);
